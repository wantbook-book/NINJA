...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    for key in ("tool_success_rate", "acc_at_k", "f1_dice_score"):
        ...
        if not numeric_values:
            continue
        metrics[f"reward/{key}/mean"] = float(np.mean(numeric_values))
        metrics[f"reward/{key}/max"] = float(np.max(numeric_values))
        metrics[f"reward/{key}/min"] = float(np.min(numeric_values))
