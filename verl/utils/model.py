...
class CausalLMWithValueHead(nn.Module):
    """Minimal value head wrapper for CausalLM critic, replaces trl.AutoModelForCausalLMWithValueHead."""
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        values = self.v_head(last_hidden).squeeze(-1)  # (batch, seq)
...
def _load_pretrained_with_attn_fallback(model_cls, local_path, torch_dtype, model_config, trust_remote_code):
    attn_impl = _get_attn_implementation(model_config)
    try:
...
def load_valuehead_model(local_path, torch_dtype, model_config, trust_remote_code):
    ...
    try:
        model = _load_pretrained_with_attn_fallback(
            AutoModelForTokenClassification, local_path, torch_dtype, model_config, trust_remote_code
        )
        return model
    ...
    ori_model = _load_pretrained_with_attn_fallback(
        module_class, local_path, torch_dtype, model_config, trust_remote_code
    )
    if hasattr(model_config, "text_config"):
        ori_model.config.hidden_size = model_config.text_config.hidden_size
    model = CausalLMWithValueHead.from_pretrained(ori_model)
