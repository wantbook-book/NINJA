...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        # used for torch.distributed.init_process_group
        if self.nnodes > 1:
            use_vllm_v1 = int(os.environ.get("VLLM_USE_V1", "0"))
            if not use_vllm_v1:
                # vLLM V0: rendezvous args are accepted as CLI flags by vllm serve
                args.update(
                    {
                ...
                # vLLM V1: serve command removed these CLI flags; pass via env vars
                # so torch.distributed.init_process_group can still find the rendezvous.
                os.environ.setdefault("MASTER_ADDR", self._master_address)
