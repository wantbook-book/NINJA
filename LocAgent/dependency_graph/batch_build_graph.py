...
def load_selected_instance_ids(instance_id_path):
    if not instance_id_path or not osp.exists(instance_id_path):
        return None
...
def _normalize_pandas_record(record):
    normalized = {}
...
def load_instances_from_local_file(dataset_spec):
    suffix = Path(dataset_spec).suffix.lower()
    if suffix != ".parquet":
        return None

    import pandas as pd

    df = pd.read_parquet(dataset_spec)
    return [_normalize_pandas_record(record) for record in df.to_dict(orient="records")]


def load_repo_folders_from_dataset(dataset_spec, split, instance_id_path):
    selected_instance_ids = load_selected_instance_ids(instance_id_path)
    selected_instance_data = {}
    repo_folders = []

    bench_data = load_instances_from_local_file(dataset_spec)
    if bench_data is None:
        bench_data = load_benchmark_dataset(dataset_spec, split)
    for instance in bench_data:
        instance_id = instance["instance_id"]
        if selected_instance_ids is not None and instance_id not in selected_instance_ids:
            continue
        repo_folders.append(instance_id)
        selected_instance_data[instance_id] = instance

    return repo_folders, selected_instance_data
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional local parquet/json/jsonl/csv file. When provided, this "
            "overrides --dataset and loads instance IDs from the file."
        ),
    )
    ...
    dataset_spec = args.input_file or args.dataset
    dataset_name = dataset_spec_to_name(dataset_spec)
    ...
    if args.download_repo:
        repo_folders, selected_instance_data = load_repo_folders_from_dataset(
            dataset_spec, args.split, args.instance_id_path
        )
    else:
        if args.input_file or osp.isfile(dataset_spec) or osp.isdir(dataset_spec):
            repo_folders, _ = load_repo_folders_from_dataset(
                dataset_spec, args.split, args.instance_id_path
            )
        else:
            selected_instance_ids = load_selected_instance_ids(args.instance_id_path)
            if selected_instance_ids is not None:
                repo_folders = sorted(selected_instance_ids)
            else:
    ...
    if total_repos > 0:
        progress_thread = threading.Thread(
            ...
            args=(progress_queue, total_repos, 'Building graph indexes'),
            daemon=True,
        )
