...
def parse_raw_loc_output(raw_output, valid_files):
    """
    For example:
    `current_file = "qwen_agent/llm/fncall.py"`
    `line = "function: build_prompt"`
    Then:
    `loc = "qwen_agent/llm/fncall.py:function: build_prompt"`
    """
    valid_top_folder = []
...
def extract_python_file_path(line, valid_folders):
    ...
    Example:
    `matched_fp = "./logs/qwen_agent/llm/fncall.py"`
    `valid_folders = ["qwen_agent", "tests"]`
    `start_index` points to the position of `"qwen_agent/"` in the string.
    Returns: `"qwen_agent/llm/fncall.py"`
    """
...
def get_edit_entities_from_raw_locs(found_edit_locs, 
    ...
    for i, edit_loc in enumerate(found_edit_locs):
        ...
        if loc.startswith("line:") or loc.startswith("lines:"):
            ...
            # `(?:...)` is a non-capturing group, used so that `?` can make the whole group optional.
            # If `loc = "lines: 12-34"`
            # `matches = [("12", "34")]`

            # If `loc = "line: 8"`
            # `matches = [("8", "")]` (the second group doesn’t match, so it becomes an empty string)

            # If `loc = "edit lines 3-5 and 10-12"`
            # `matches = [("3", "5"), ("10", "12")]`
